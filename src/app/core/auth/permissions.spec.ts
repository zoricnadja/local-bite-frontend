...
@Component({ imports: [CanDirective], template: `<button *appCan="'shop'">New order</button><button *appCan="'manageProducts'">Edit product</button><button *appCan="'deleteBusinessData'">Delete</button><button *appCan="'analytics'">Analytics</button>` })
class ActionsHost {}
describe('Role permissions', () => {
  it('updates visible actions for customer, owner and worker', async () => {
    ...
    role.set('BusinessOwner'); fixture.detectChanges();
  ...
  it('excludes customer from business features and owner from purchasing', () => {
    expect(hasPermission('Customer', 'viewBusiness')).toBe(false);
    expect(hasPermission('Customer', 'manageProduction')).toBe(false);
    expect(hasPermission('Customer', 'viewMaterials')).toBe(false);
    expect(hasPermission('BusinessOwner', 'shop')).toBe(false);
